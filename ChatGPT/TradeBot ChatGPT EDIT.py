...
def moving_average_crossover_analysis(stock_data, short_window, long_window):
    ...
    signals['SMA_short'] = SMAIndicator(stock_data['Close'], window=short_window).sma_indicator()

    # Create long simple moving average
    signals['SMA_long'] = SMAIndicator(stock_data['Close'], window=long_window).sma_indicator()

    # Generate trading signals based on the crossover
    signals['signal'][short_window:] = np.where(signals['SMA_short'][short_window:] > signals['SMA_long'][short_window:], 1.0, 0.0)
...
# Plot additional indicators
    ax.plot(stock_data.index, signals['SMA_short'], label='Short MA', linestyle='--')
    ax.plot(stock_data.index, signals['SMA_long'], label='Long MA', linestyle='--')

    ax2 = ax.twinx()
    ax2.plot(stock_data.index, RSIIndicator(stock_data['Close'], window=14).rsi(), label='RSI', color='orange', alpha=0.5)
    ax2.axhline(70, color='r', linestyle='--', alpha=0.5, label='Overbought')
    ax2.axhline(30, color='g', linestyle='--', alpha=0.5, label='Oversold')

    ax3 = ax.twinx()
    ax3.plot(stock_data.index, DonchianChannel(stock_data['High'], stock_data['Low'], stock_data['Close'], window=20).donchian_channel_hband(), label='Upper Band', linestyle='-.', color='purple', alpha=0.5)
    ax3.plot(stock_data.index, DonchianChannel(stock_data['High'], stock_data['Low'], stock_data['Close'], window=20).donchian_channel_lband(), label='Lower Band', linestyle='-.', color='brown', alpha=0.5)
